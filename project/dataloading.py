...
def load_data_from_files(filepath):
...
# def basic_embed(df, author_embedding, content_embedding):
#     for i, row in df.iterrows():
#         author, content = row
...
def load_data(path, embedding="basic"):
    if os.path.exists(os.getcwd() + f'/{embedding}_embed.pkl'):
        with open(os.getcwd() + f'/{embedding}_embed.pkl', 'rb') as file:
            df = pickle.load(file)
    else:
        df = load_data_from_files(path)
        ...
        elif embedding == "padded":
            padded_embed(df, max_length, author_embedding, content_embedding)
        # elif embedding == "basic":
        #     basic_embed(df, max_length, author_embedding, content_embedding)
        else:
            raise Exception("Improper Embedding Type")
        with open(os.getcwd() + f'/{embedding}_embed.pkl', 'wb') as file:
            pickle.dump(df, file)
    return df
